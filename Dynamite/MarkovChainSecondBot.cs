...
namespace Dynamite
{
    public class MarkovChainSecondBot : IBot
    {
        #region Fields
        ...
        readonly int[,,] _markovChain = new int[5, 5, 5];
        ...
        void UpdateMarkovChain(Move prev2, Move prev, Move next)
        {
            _markovChain[_ordinals[prev2],_ordinals[prev], _ordinals[next]]++;
        }
        
        private Move PredictNextMove(Move prev2Move, Move previousMove)
        {
            int nextIndex = 0;

            for (int i = 0; i < _ordinals.Count; i++)
            {
                int prev2Index = _ordinals[prev2Move];
                int prevIndex = _ordinals[previousMove];

                if (_markovChain[prev2Index,prevIndex, i] > _markovChain[prev2Index,prevIndex, nextIndex])
                {
                    nextIndex = i;
                }
            }
        ...
        public Move MakeMove(Gamestate gamestate)
        {
            ...
            if (history.Count() >= 3)
            {
                var threeMovesAgo = history.ElementAt(history.Count() - 3).GetP2();
                UpdateMarkovChain(threeMovesAgo, twoMovesAgo,previousMove);
            }

            var predictedNext = PredictNextMove(twoMovesAgo,previousMove);
